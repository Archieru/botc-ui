...
class NightActivity : BaseGameActivity() {
    ...
    private fun poisoner() {
        if (!game.players.any { it.role!!.name == RoleName.Poisoner }) { nextRole() }
        binding.contentText.text = "poisoner"
    }

    private fun washerWoman() {
        if (!game.players.any { it.role!!.name == RoleName.WasherWoman }) { nextRole() }
        binding.contentText.text = "washerWoman"
    }

    private fun librarian() {
        if (!game.players.any { it.role!!.name == RoleName.Librarian }) { nextRole() }
        binding.contentText.text = "librarian"
    }

    private fun investigator() {
        if (!game.players.any { it.role!!.name == RoleName.Investigator }) { nextRole() }
